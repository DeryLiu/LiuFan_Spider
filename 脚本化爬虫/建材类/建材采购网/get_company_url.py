...
def handle():
    ...
    lock = Lock()
    cate_all_url = open('company_all_url1.csv','w')
    error_file = open('error_file1.csv','w')

    info_tuple_list = []
    with open('cate_all_url.csv','r') as cate_url_file:
...
def get_info(info_list):
    ...
    try:
        req = requests.get(basic_url,headers=get_headers(),proxies=get_proxies_ip(),timeout=30)
        # req = requests.get(basic_url)
        req.encoding = 'gb2312'
        soup = BeautifulSoup(req.text,'html.parser')
        company_info = soup.select('div.leftContent > div.img_title > dl > dd > a.a6')
        for company in company_info:
            company_url = company['href']
            company_titel = company.text

            lock.acquire()
            cate_all_url.write(firs_cate+','+sec_cate+','+company_titel+','+company_url+','+basic_url+'\n')
            cate_all_url.flush()
            lock.release()
            print(basic_url)
...
#     company_info = soup.select('div.leftContent > div.img_title > dl > dd > a.a6')
#     for company in company_info:
#         print(company['href'],company.text)
    # print(company_info)
