...
def log_error(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            with codecs.open('log_%s.log' % get_date(), 'a', encoding='utf-8') as f:
                f.write(msg)
        return result
...
class Spider(object):
    ...
    def load_configs(self, path):
        ...
        # Load files
        self.load_category_keys(os.path.join(base_path, conf.get('files', 'category_keys')))
        self.category_urls_file = conf.get('files', 'category_urls')
        self.product_urls_file = conf.get('files', 'product_urls')
        self.load_product_keys(os.path.join(base_path, conf.get('files', 'product_keys')))
        self.load_uas(os.path.join(base_path, conf.get('files', 'ua_file')))
        self.load_proxies(os.path.join(base_path, conf.get('files', 'proxy_file')))
        self.load_headers(os.path.join(base_path, conf.get('files', 'header_file')))
            ...
            try:
                ...
                if retry_word is not None and retry_word in html:
                    ...
                    if store_page:
                        ...
                        with codecs.open(store_folder + url.replace('/', '_') + '.html', 'w', encoding='utf-8') as f:
                            f.write(html)
                    break
                ...
                html = None
        if html is None:
            with codecs.open('fail_urls.txt', 'a', encoding='utf-8') as f:
                f.write('{0}\n'.format(url))
        return html
