import requests,re,pymysql,random
...
def handle():
    ...
    lock = Lock()
    company_file = open('all_compang_url.csv','w')
    error_file = open('error_file1.csv','w')
    company_url = open('compang_all_url.csv','r')
    # company_file = open('all_compang_url_buchong.csv','w')
    # error_file = open('error_file1.csv','w')
    # company_url = open('error_file.csv','r')
    info_tuple_list = []
    for comp_url in company_url.readlines():
        firt_cate = comp_url.split('\t')[0]
        sec_cate = comp_url.split('\t')[1]
        thir_cate = comp_url.split('\t')[2]
        comp_url = comp_url.split('\t')[3].strip()
        # comp_url = comp_url.split('\t')[4].strip()
        info_tuple_list.append((firt_cate,sec_cate,thir_cate,comp_url))
...
def get_info(info_tuple_list):
    ...
    comp_url = info_tuple_list[3]
    # company_url = info_tuple_list[4]
    # print(company_url)
    try:
        req = requests.get(comp_url,headers=get_headers(),proxies=get_proxies_ip(),timeout=30)
        req.encoding = 'gb2312'
        ...
        for company_info in company_info_list:
            ...
            company_file.flush()
            lock.release()
            print(company_name,comp_url)
    except Exception as e:
        lock.acquire()
        error_file.write(firt_cate+','+sec_cate+','+thir_cate+','+comp_url+'\n')
        error_file.flush()
        lock.release()
        print(e)
