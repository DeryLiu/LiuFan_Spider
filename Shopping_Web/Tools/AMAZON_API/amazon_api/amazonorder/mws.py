...
class MWS(object):
    ...
    def __init__(self, access_key, secret_key, account_id,mws_auth_token,region='US', domain='', uri="", version=""):
...
#             start_time = datetime.datetime.now()
            ...
            response = request(method, url, data=kwargs.get('body', ''), headers=headers, timeout=30)
...
class InboundShipments(MWS):
    ...
    def list_inbound_shipments(self,LastUpdatedAfter = None,LastUpdatedBefore = None):
        ...
        data.update(self.enumerate_param('ShipmentStatusList.member', ['WORKING']))
        
        return self.make_request(data,"POST")
    
    def list_inbound_shipments_by_nexttoken(self,NextToken = None):
    ...
    def list_inbound_shipment_items(self,ShipmentId = None):
    ...
    def list_inbound_shipment_items_by_nexttoken(self,NextToken = None):
...
class Subscriptions(MWS):
    ...
    Destination = {'key':'sqsQueueUrl','Value':'https://sqs.us-east-1.amazonaws.com/51471EXAMPLE/mws_notifications'}
    
    
    def registerDestination(self,marketplaceid):
        ...
        data.update({'Destination.AttributeList.member.1.Value':'https://sqs.us-east-1.amazonaws.com/51471EXAMPLE/mws_notifications'})
        data.update({'Destination.DeliveryChannel':'SQS'})
        return self.make_request(data, "POST")
    
    
    def testNotificationToDestination(self,marketplaceid):
        data = dict(Action="SendTestNotificationToDestination",MarketplaceId=marketplaceid)
        data.update({'Destination.AttributeList.member.1.Key':'sqsQueueUrl'})
        data.update({'Destination.AttributeList.member.1.Value':'https://sqs.us-east-1.amazonaws.com/51471EXAMPLE/mws_notifications'})
        data.update({'Destination.DeliveryChannel':'SQS'})
        return self.make_request(data, "POST")
    ...
    def getListRegisteredDestinations(self,marketplaceid):
        data = dict(Action="ListRegisteredDestinations",MarketplaceId=marketplaceid)
        return self.make_request(data, "POST")
