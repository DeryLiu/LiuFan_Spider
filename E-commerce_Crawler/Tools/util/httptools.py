...
#        self.__head['Host']='www.amazon.com'
        ...
        self.__head['User-Agent']='Mozilla/5.0 (X11; Linux x86_64; rv:38.0) Gecko/20100101 Firefox/38.0'
...
#         proxy_support = urllib2.ProxyHandler({'http':'http://50.2.15.204:8800'})
        ...
        while True:
            try: 
                ...
                content = res.read()  
                if -1== content.find('Sorry, we just need to make sure you') or count > 5:#<title dir="ltr">Amazon CAPTCHA</title>
                    count+=1
                    return content
            except Exception,e:
                print str(e)
    ...
    def geturlproxy(self,url,data=None,heads='FHBJ<KGFYOLL'):
        if not self.__proxylist:
            if os.path.exists('./myutil/proxylist.txt'):
                f=open('./myutil/proxylist.txt')
            else:
